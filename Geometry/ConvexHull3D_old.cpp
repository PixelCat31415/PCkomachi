// (FP) needs 3D basic
// requires at least four noncoplanar points
...
// all points coplanar case will WA, O(n^2)
  ...
  swap(P[1], *find_if(iter(P), [&](auto p) {
    return sign((P[0] - p) * (P[0] - p)) != 0;
  }));
  swap(P[2], *find_if(iter(P), [&](auto p) {
    auto v = cross3(p, P[0], P[1]);
    return sign(v * v) != 0;
  }));
  swap(P[3], *find_if(iter(P), [&](auto p) {
...
bool same(Face s, Face t) {
  if (sign(volume(P[s.a], P[s.b], P[s.c],
      P[t.a])) != 0) return 0;
  if (sign(volume(P[s.a], P[s.b], P[s.c],
      P[t.b])) != 0) return 0;
  if (sign(volume(P[s.a], P[s.b], P[s.c],

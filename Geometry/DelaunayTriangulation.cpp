// (int) needs point in circle
const int N = 0; // set per problem constraints
// points are distinct and strictly inside the root
...
*/
const ll inf = 1e9; // not too small and not too big
struct Tri;
struct Edge {
  Tri* tri; int side;
  Edge(): tri(0), side(0){}
...
struct Trig { // Triangulation
  Trig() {
    the_root = // Tri must contain all points
      new(tris++) Tri({-inf, -inf}, {inf + inf, -inf},
        {-inf, inf + inf});
  }
  ...
  void add_point(Tri* root, PT p) {
    Tri* t[3];
    /* split it into three triangles */
    for (int i = 0; i < 3; ++i)
      t[i] = new(tris++) Tri(root->p[i],
        root->p[(i + 1) % 3], p);
    for (int i = 0; i < 3; ++i)
  ...
  void flip(Tri* tri, int pi) {
    ...
    tri->chd[0] = trk; tri->chd[1] = trl;
    tri->chd[2] = 0;
    trj->chd[0] = trk; trj->chd[1] = trl;
    trj->chd[2] = 0;

auto sais(const auto &s) { // SCOPE HASH
  ...
  for (auto x = c; int i : I | is_lms)
    q[i] = sz(lms), lms.push_back(sa[--x[s[i]]] = i);
  ...
  fill(iter(sa), 0); auto nsa = sais(ns);
  for (auto x = c; int y : nsa | views::reverse)
    y = lms[y], sa[--x[s[y]]] = y;
  return induce(), sa;
}
struct Suffix {
  int n; vector<int> sa, lcp, ra;
  Suffix(const auto &_s, int _n) : n(_n), lcp(n), ra(n) { // SCOPE HASH
    vector<int> s(n + 1); // s[n] = 0;
    copy_n(_s.begin(), n, begin(s)); // _s shouldn't contain 0

...
void solve(vector<pii> &E) {
  ...
  for (int t = 0; t < E.size(); ++t) {
    int u = E[t].F, v0 = E[t].S, v = v0, c0 = X[u], c = c0, d;
    vector<pii> L;
    fill_n(vst + 1, n, 0);
    while (!G[u][v0]) {
      L.emplace_back(v, d = X[v]);
      if (!C[v][c]) for (int a = (int)L.size() - 1; a >= 0; --a) c = color(u, L[a].F, c);
      else if (!C[u][d]) for (int a = (int)L.size() - 1; a >= 0; --a) color(u, L[a].F, L[a].S);
      else if (vst[d]) break;
    ...
    if (!G[u][v0]) {
      ...
      if (int a; C[u][c0]) {
        for (a = (int)L.size() - 2; a >= 0 && L[a].S != c; --a);
